...
//    }
    ...
    fun Context.openPrivacy() {
        openUrl("https://sites.google.com/view/mealscanai-caloriecounter/privacy-policy")
    }
...
//        Log.d("===Event", eventName + "_" + BuildConfig.VERSION_CODE)
    ...
    fun getAccessToken(appContext: Context): String {
        val preferences = appContext.getSharedPreferences(appContext.packageName, MODE_MULTI_PROCESS)
        return preferences.getString("KEY_ACCESS_TOKEN", "") ?: ""
    }


    fun setAccessToken(appContext: Context, input: String) {
        val preferences = appContext.getSharedPreferences(appContext.packageName, MODE_MULTI_PROCESS)
         preferences.edit().putString("KEY_ACCESS_TOKEN", input).apply()
    }
